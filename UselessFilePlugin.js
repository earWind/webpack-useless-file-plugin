...
// 查找没有用到的文件
class UselessFilePlugin {
  constructor(options) {
    this.opts = options
  }
  apply(compiler) {
    compiler.hooks.emit.tap('useless-file', (compilation) => {
      this.findUnusedFiles(compilation)
    })
  }
  ...
  async findUnusedFiles(compilation) {
    ...
    try {
      ...
      let unUsed = allFiles.filter((item) => {
        const isvj = item.includes('.vue') || item.includes('.js')
        return isvj && !~allChunks.indexOf(item)
      })
...
module.exports = UselessFilePlugin
